...
//         let mut delta: i128 = i16::from_le_bytes([
//             block[initial_delta_index],
//             block[initial_delta_index + 0x01],
//         ]) as i128;
//
//         let sample1_index = total_channels + total_channels * 0x02 + current_channel * 0x02;
//         let mut sample1: i128 =
//             i16::from_le_bytes([block[sample1_index], block[sample1_index + 0x01]]) as i128;
//         let sample2_index = sample1_index + total_channels * 0x02;
//         let mut sample2: i128 =
//             i16::from_le_bytes([block[sample2_index], block[sample2_index + 0x01]]) as i128;
//
...
//     };
//
//     predictor = ((block_header.sample1 * block_header.coeff1)
//         + (block_header.sample2 * block_header.coeff2))
//         >> 8;
...
//     block_header.delta =
//         ((MSADPCM_STEPS[(nibble & 0xf) as usize] as i128) * block_header.delta) >> 8;
//     if block_header.delta < 16 {
//         block_header.delta = 16
//     };
//
...
//     add_u2_to_u8_vec(
//         &mut temp_buffer,
//         (left_channel_block_header.sample2 as i16).to_ne_bytes(),
//     );
//     add_u2_to_u8_vec(
//         &mut temp_buffer,
//         (right_channel_block_header.sample2 as i16).to_ne_bytes(),
//     );
//
//     add_u2_to_u8_vec(
//         &mut temp_buffer,
//         (left_channel_block_header.sample1 as i16).to_ne_bytes(),
//     );
//     add_u2_to_u8_vec(
//         &mut temp_buffer,
//         (right_channel_block_header.sample1 as i16).to_ne_bytes(),
//     );
//
...
//     add_u2_to_u8_vec(
//         &mut temp_buffer,
//         (mono_channel_block_header.sample2 as i16).to_ne_bytes(),
//     );
//     add_u2_to_u8_vec(
//         &mut temp_buffer,
//         (mono_channel_block_header.sample1 as i16).to_ne_bytes(),
//     );
//
...
//
//         for shift in 0..2 {
//             let mono_channel_decoded_block =
//                 decode_byte(&mut mono_channel_block_header, byte, shift);
...
// }
//
// pub fn decode<R: BufferReader>(metadata: &AssetSCDFile, buffer: &mut Buffer<R>) -> Vec<u8> {

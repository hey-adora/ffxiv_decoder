...
impl<'a> Asset<'a, EXH> {
    ...
    pub fn get_page(
        ...
        lang: &EXHLang,
        page: u32,
    ) -> Result<(DatPath, String), AssetEXHGetPageError> {
        let exd_asset_path =
            Asset::make_exd_path(&self.path.path_dir, &self.path.path_stem, lang, page);
    ...
    pub fn get_pages(
        &self,
        lang: &EXHLang,
    ) -> Result<Vec<(DatPath, String)>, AssetEXHGetPageError> {
...
impl<'a> Asset<'a, EXD> {
    pub fn new_exd(
        ...
        exd_path: DatPath,
        exh: &EXH,
    ) -> Result<Asset<'a, EXD>, AssetNewError> {
...
impl<'a> Asset<'a, AssetSCDFile> {
    pub fn new_scd(
        ffxiv: &'a FFXIV,
        path: DatPath,
    ) -> Result<Asset<'a, AssetSCDFile>, AssetNewError> {
        let asset = ffxiv.get_asset_standard(&path)?.decompress()?;
        let data = AssetSCDFile::from_vec(asset);

...
impl AssetSCDFile {
    pub fn new<R: BufferReader>(buffer: &mut Buffer<R>) -> AssetSCDFile {
        buffer.offset_set(0);
        ...
        let entry_stream_size = buffer.be_le_i32_at((entry_offset) as u64, is_big_endian);
        let entry_channels = buffer.be_le_i32_at((entry_offset + 0x4) as u64, is_big_endian);
        let entry_sample_rate = buffer.be_le_i32_at((entry_offset + 0x8) as u64, is_big_endian);
